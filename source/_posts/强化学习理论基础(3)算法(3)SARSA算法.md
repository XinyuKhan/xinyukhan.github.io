...
前面我们介绍了如何用TD算法训练DQN，具体内容参见[《使用TD算法训练DQN》](/2025/08/12/强化学习理论基础(3)算法(1)使用TD算法训练DQN/)和[《使用TD算法训练DQN方法改进》](/2025/08/12/强化学习理论基础(3)算法(2)使用TD算法训练DQN方法/)，该方法被称为Q学习算法，根据介绍我们不难发现，Q学习算法的目标是去近似一个最优动作价值函数 $Q^*(s, a)$ ，其训练过程通常是利用一个四元组 $s_t, a_t, r_t, s_{t+1}$ 来更新学习参数，这个过程是不依赖产生四元组的具体策略的。因此Q学习属于 **异策略（Off Policy）** 算法，可以使用经验回放。
