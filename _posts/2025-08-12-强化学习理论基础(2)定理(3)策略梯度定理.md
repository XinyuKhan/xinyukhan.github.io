...
其中$p$是状态$s$和动作$a$下后续状态的转移概率分布，$R$是根据状态$s$、动作$a$和后续状态$s'$获取的奖励，这些都只跟环境和奖励设计有关，跟策略本身无关，因此2.1.2中的2.1.3部分对于$\boldsymbol{\theta}$的梯度为$\boldsymbol{0}$，2.1.4部分的状态转移条件概率函数可以提出来，于是有
