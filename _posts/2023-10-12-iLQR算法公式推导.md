...
# 背景
在之前的文章[《离散系统的LQR推导》](2023-09-27-离散系统的LQR推导.md)中，我们讨论了LQR在线性系统中的应用，我们讨论了该算法在线性时变系统中的一般情况，也讨论了在线性时不变系统中的特殊情况；以及**有限时域（Finite Horizon）**和**无限时域（Infinite Horizon）**这两种情况。不过在实际的控制系统中，大部分系统的模型都是非线性的，传统的LQR只能在系统的当前状态下对系统进行线性化近似，这种近似并不能保证基于线性假设得到的最优控制率实际上真的是最优的。
...
根据[《离散系统的LQR推导》](2023-09-27-离散系统的LQR推导.md)的中的公式（8）中的相关推导，我们可以得到**cost to go**：
